import {
  ...
  external_exports,
  generateObject
} from "../../../chunk-KBYJ7EGZ.mjs";
import "../../../chunk-GL53E3WX.mjs";
import {
  task
} from "../../../chunk-4HKZKBNS.mjs";
...
// lib/liveblocks.ts
init_esm();
var globalForLiveblocks = global;
var liveblocks = globalForLiveblocks.liveblocks ?? new Liveblocks({ secret: process.env.LIVEBLOCKS_SECRET_KEY ?? "" });
...
var VALID_SHAPES = ["rectangle", "diamond", "circle", "pill", "cylinder", "hexagon"];
var VALID_COLORS = NODE_COLORS.map((c) => c.fill);
var OperationSchema = external_exports.discriminatedUnion("action", [
  external_exports.object({
    action: external_exports.literal("add_node"),
    id: external_exports.string(),
    label: external_exports.string(),
    shape: external_exports.enum(VALID_SHAPES),
    color: external_exports.string(),
    x: external_exports.number(),
    y: external_exports.number(),
    width: external_exports.number().default(160),
    height: external_exports.number().default(80)
  }),
  external_exports.object({
    action: external_exports.literal("update_node"),
    id: external_exports.string(),
    label: external_exports.string().optional(),
    color: external_exports.string().optional(),
    shape: external_exports.enum(VALID_SHAPES).optional()
  }),
  external_exports.object({
    action: external_exports.literal("move_node"),
    id: external_exports.string(),
    x: external_exports.number(),
    y: external_exports.number()
  }),
  external_exports.object({
    action: external_exports.literal("resize_node"),
    id: external_exports.string(),
    width: external_exports.number(),
    height: external_exports.number()
  }),
  external_exports.object({
    action: external_exports.literal("delete_node"),
    id: external_exports.string()
  }),
  external_exports.object({
    action: external_exports.literal("add_edge"),
    id: external_exports.string(),
  ...
  external_exports.object({
    action: external_exports.literal("delete_edge"),
    id: external_exports.string()
  })
]);
...
var designAgentTask = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      const storageJson = await liveblocks.getStorageDocument(roomId, "json");
      const flowData = storageJson?.flow;
      ...
      const { object } = await generateObject({
        ...
        prompt: [
          ...
          existingNodes.length > 0 ? `Existing nodes:
${JSON.stringify(existingNodes, null, 2)}` : "Canvas is empty.",
          existingEdges.length > 0 ? `Existing edges:
        ...
        for (const op of operations) {
          switch (op.action) {
            case "add_node": {
              ...
              const shape = VALID_SHAPES.includes(op.shape) ? op.shape : "rectangle";
              const node = {
                id: op.id,
                type: "canvasNode",
              ...
              nodesMap.set(op.id, LiveObject.from(node, NODE_SYNC_CONFIG));
              break;
            }
            case "update_node": {
              ...
              if (op.label !== void 0) data.set("label", op.label);
              if (op.color !== void 0 && VALID_COLORS.includes(op.color))
                data.set("color", op.color);
              if (op.shape !== void 0 && VALID_SHAPES.includes(op.shape))
            ...
            case "add_edge": {
              ...
              edgesMap.set(op.id, LiveObject.from(edge, EDGE_SYNC_CONFIG));
              break;
            }
            case "delete_edge": {
